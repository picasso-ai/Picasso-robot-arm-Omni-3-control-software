...
const char *ssid = "Picasso_OMNI3"; // should be connected Wi-Fi's name
const char *password = "Picasso1234"; // should be connected Wi-Fi's password
const char *hostname = "espressif";
IPAddress local_IP(192, 168, 1, 211); // static ip addtress to be assigned to ESP32 board
IPAddress gateway(192, 168, 1, 0);    // Gateway of the connected network
IPAddress subnet(255, 255, 255, 0);   // subnet code of connected network

WebSocketsServer webSocket = WebSocketsServer(81); // webscoket port
uint16_t status;
...
void webSocketEvent(uint8_t num, WStype_t type, uint8_t *payload, size_t length) {
    switch (type) {
        ...
        case WStype_CONNECTED: {
            ...
            Serial.printf("[%u] from %d.%d.%d.%d connect\n", num, ip[0], ip[1], ip[2], ip[3]);
            webSocket.sendTXT(num, "Hello from ESP32");
            break;
        }
        case WStype_TEXT:
            Serial.printf("[%u] receive text: %s\n", num, payload);
            ...
            if (!error) {
                ...
                if (doc.containsKey("motors")) {
                    ...
                    webSocket.sendTXT(num, "Motor command executed");
                }
                // Set up limitation
                if (doc.containsKey("setLimit")) {
                    ...
                    Serial.printf("Limitation set: Motor %d, Type %d, Value %.2f\n", motorId, limitType, value);
                    webSocket.sendTXT(num, "Limitation command executed");
                }
                //Read limitations
                ...
                if (doc.containsKey("estop")) {
                    ...
                    rc_estop(motorId);
                    webSocket.sendTXT(num, "Estop activated");
                }
                // Set motor status
                ...
                if (doc.containsKey("setPID")) {
                    ...
                    webSocket.sendTXT(num, "PID parameters set");
                }
                // Read PID parameters
                ...
                if (doc.containsKey("singleRun")) {
                    ...
                    webSocket.sendTXT(num, "Single point run executed");
                }
                ...
                if (doc.containsKey("setCTPosition")) {
                    ...
                    webSocket.sendTXT(num, "Continuous track position data has been set");
                }
                // Continuous trajectory velocity data
                if (doc.containsKey("setCTSpeed")) {
                    ...
                    webSocket.sendTXT(num, "Continuous track speed data has been set");
                }
                // Continuous trajectory moment flow data
                if (doc.containsKey("setCTTorque")) {
                    ...
                    webSocket.sendTXT(num, "Continuous trajectory moment flow data is set");
                }
                // Run as the specified trajectory data
                if (doc.containsKey("runTrajectory")) {
                    ...
                    webSocket.sendTXT(num, "The specified trajectory data run has been executed");
                }
                // Record data to the specified index location
                if (doc.containsKey("recordData")) {
                    ...
                    webSocket.sendTXT(num, "record data completed");
                }
                // Read run data
                if (doc.containsKey("readRunData")) {
                    ...
                    char buffer[64];
                    sprintf(buffer, "Run data Motor %d, Parameter %d is %.2f", motorId, parameter, value);
                    webSocket.sendTXT(num, buffer);
                }
                // Set up CAN ID
                if (doc.containsKey("setCANID")) {
                    ...
                    webSocket.sendTXT(num, "CAN ID set up");
                }
                // Reset settings
                if (doc.containsKey("resetSettings")) {
                    int motorId = doc["resetSettings"]["motorId"];
                    rc_reset(motorId);
                    webSocket.sendTXT(num, "settings are reset");
                }
            } else {
...
void loop()
{
    ...
    webSocket.loop();
    String message;  // Define once, for the whole function
}
